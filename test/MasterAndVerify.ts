...
import { signAndEncodeChainProof, signTypedData, typedData } from "./lib/eip712SigUtils";
import { EIP712Domain, ProofData } from "./lib/types";
const { mineBlock } = require("./hardhat-func");
...
async function fixture(_wallets: Wallet[]) {
...
describe("Master", async () => {
  let owner: SignerWithAddress, signer: Wallet, addr1: SignerWithAddress, addr2: SignerWithAddress, master: Solve3Master, verify: Solve3VerifyMock;
  beforeEach(async () => {
    ({ owner, signer, addr1, addr2, master, verify } = await loadFixture(fixture));
  });
