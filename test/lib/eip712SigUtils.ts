...
export const signTypedData = (signer: ethers.Wallet, typedData: TypedData): string => {
  return sign(Buffer.from(signer.privateKey.slice(2), 'hex'), { data: typedData })
}
...
export const signAndEncodeChainProof = async (signer: ethers.Wallet, typedData: TypedData): Promise<string> => {
  let sig = signTypedData(signer, typedData);
